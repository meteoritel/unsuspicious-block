...
> P2 三项与部分 P3 已对齐技术路线（四层模型：快照 → 引用图 → 编译 → 投影，分三步增量落地），实施内容见 [战利品表解析追踪重构实施计划](../plan/loottable-refactor-plan.md)。
...
| P2-1 | 中 | 引用图遍历重复实现 4 套，循环处理 3 套，类型判定写法不一致 | 路线已定，待实施（步骤①） |
| P2-2 | 中 | 概率为字符串（P1-1 的根因），无法数值聚合 | 路线已定，待实施（步骤③） |
| P2-3 | 中 | 解析态与模拟态共用记录类型，非法状态可表示 | 路线已定，待实施（步骤②） |
| P3-1 | 低 | 网络包按物品重复序列化同一场景的假设条件树 | 路线已定，随步骤② 一起做 |
| P3-2 | 低 | 运行时签名匹配缺预览栈缓存（模拟热路径有） | 待办 |
| P3-3 | 低 | 模拟失败任务不会重试，日志措辞误导 | 待办 |
| P3-4 | 低 | fishing / mud_dredging 硬编码关系散落三处 | 待办 |
...
**残留注意**：P1-2 的运行时守卫会对"场景控制类型但未命中覆盖"的条件告警，其中"条件来自运行时注入路径"（如其他模组 GLM 注入、被排除的循环引用表）属预期命中，看到告警时先按这条排除。
