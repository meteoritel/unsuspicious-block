...
| P3-2 | 低 | 运行时签名匹配缺预览栈缓存（模拟热路径有） | **已修复**：`LootResultPreviewCache` 按签名内容缓存，玩家侧四条匹配路径共用；重载时释放 |
| P3-3 | 低 | 模拟失败任务不会重试，日志措辞误导 | **已修复**：如实说明"不重试 + 概率保持未知"，并在队列排空时汇总列出失败表 |
| P3-4 | 低 | fishing / mud_dredging 硬编码关系散落三处 | **已修复**（步骤①）：收敛到 `RuntimeLootLinks`（含条件类型 id），钓鱼上下文改读声明的 `type` |
| P3-5 | 低 | 两套"可信度"口径（tooltip 颜色 vs `"?"`）规则不同 | **已修复**：等级判定收敛到 `ItemDefinition.uncertaintyLevel()`（与 `hasConditions()` 同源），未知概率统一按"未知"着色 |
