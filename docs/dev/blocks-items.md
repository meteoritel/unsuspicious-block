...
- **花火粉**：火把花研磨的中间材料，可作燃料、合成火把与回溯粉。
- **回溯粉**：对世界生成结构原位重生成（`world/StructureRewindService`），见 3.9 节。
...
- **合成**：普通 3×3 工作台使用 7 个回响碎片、1 个花火粉和 1 瓶龙息。
...
### 3.10 花火粉（SparkDustItem）

[`ModItems`](../../common/src/main/java/com/meteorite/unsuspiciousblock/item/ModItems.java) 注册的中间材料（`DescribedItem`，无独立类），为回溯粉的合成原料：

- **合成**：2 个火把花无序合成 4 个花火粉。
- **燃料**：燃烧 800t，恰好烧炼 4 个物品。双平台接入：Fabric 用 `FuelRegistry`，NeoForge 用 `FurnaceFuelBurnTimeEvent`（接入点在两端入口类）。
- **合成火把**：1 花火粉 + 1 木棍 → 2 个火把。**刻意不加入 `minecraft:coals` 标签**——加入后会被原版火把配方识别为煤炭，产出变 4 个。
