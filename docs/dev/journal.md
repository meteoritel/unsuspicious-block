...
4. **日志**：记录每次发现的物品、坐标、维度、群系、结构、时间，支持分组、备注、复制传送指令、表级自动保留和单条/批量/整表删除。
...
- 每张表持久化玩家设置的 `retention_limit` 与 `long lifetime_entry_count`。前者受服务器 `maxLogEntriesPerTable` 全局兜底限制；后者只在新 `entryId` 首次写入时累计，普通 GUI 删除不会回退，`/usb journal clear [table_id]` 才会连同表历史一起清除。
- 新增日志时，如果带备注条目数已经达到当前表有效上限，则拒绝新增并警告玩家；否则自动删除最旧的无备注条目直到回到上限。一次性“仅保留最近 N 条”同样保护所有带备注条目。玩家后来降低上限时允许带备注条目数暂时超过上限，但在清除备注或删除日志前不能新增。

#### 4.4.1 版本迁移约定

考古日志有两个互不混用的版本轴，统一定义在 `JournalDataVersion`：

- **NBT schema 版本**：描述字段与嵌套结构，当前为 v2；缺少 `data_version` 视为 v0。v2 为单表历史增加自动保留上限与 `long` 累计条目数。
...
玩家可在详情页删除单条日志，也可在列表进入批量选择模式后选择条目或整个分组；批量执行、清空当前表和清空全部表都使用 `ConfirmScreen` 二次确认。配置页还支持按更新时间仅保留最近 N 条，以及设置当前表自动保留上限。C2S 请求只能操作连接玩家自身的数据。显式的单条、批量、当前表和全部表删除会无视备注保护，但只删除日志条目，不回退累计数、保留策略、目录解锁、物品计数、完成奖励或成就。
