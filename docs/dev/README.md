...
| 玩法子系统 | [进度（成就）系统](advancement.md) | 进度 JSON 组织 / 触发器与代码授予 / 收集类判定 / 平台抽象与补发 |
...
| 新增 / 修改进度条目 | [进度（成就）系统](advancement.md) 第 6 节 |
...
| `docs/plan/` | 实施计划 | 进行中的子系统改造规划；实施完成后移入 `docs/archive/`。写作规范见下文《计划文档统一结构》 |
| `docs/archive/loot-performance/` | 本地观测记录 | **本地目录，不入库**（随 `docs/archive/` 一并忽略）。性能归因的原始日志、逐表 CPU 对照与操作说明，含本机路径与存档摘要 |
| `docs/todo/` | 审查与待办 | 代码审查结论、问题清单与待办状态；**机器可读的过程记录，不构成机制权威**（机制一律以 `docs/dev/` 为准），条目完成后同步到本目录对应文档或直接删除 |
| `docs/archive/` | 归档 | **本地目录，不入库**（列在 `.gitignore`）。已实施完成的计划文档保留在其中，含迁移背景、技术核查与决策记录；当前机制一律以 `docs/dev/` 为准 |
| `CONTEXT.md`（项目根目录） | 领域语言 | **本地文件，不入库**。各玩法子系统共用的统一术语权威，只服务于本地开发与 AI 协作；入库文档需要术语时以文本形式就地定义，不要指向它 |
| `AGENTS.md` / `CLAUDE.md`（项目根目录） | 开发准则 | **本地文件，不入库**。项目开发约束的本地副本 |
...
- **不改向未入库的本地文件**：`CONTEXT.md`、`AGENTS.md`、`CLAUDE.md`、`INSPIRATION.md`、`docs/archive/`（含 `docs/archive/loot-performance/`）都在 `.gitignore` 里，克隆仓库的人看不到它们。本目录文档必须遵循同样的写法：不得用 markdown 链接指向这些路径——链接在仓库里是死链。需要引用其中的内容时，把必要的那部分**以文本形式写进本文档**，只把本地文件当作本地环境的参考，而不是文档依赖；本地目录本身可以用行内代码写出路径，但不做链接。

### 计划文档统一结构（`docs/plan/`）

`docs/plan/` 存放进行中的子系统改造规划，实施完成后移入 `docs/archive/`。结构参考已归档的 `advancement-tree-plan.md`、`panning-variants.md`、`shimmer-worldgen-feature.md`，统一为九章：

| 章 | 章节 | 必填 | 内容要求 |
|---|---|---|---|
| — | 状态块 | 是 | 文档顶部引用块：状态与日期、与 `docs/dev/` 权威文档的关系（计划文档不构成机制权威）、修订记录 |
| 一 | 现状 | 是 | **实施前**的真实状态：现有实现（组件 / 文件 / 职责）、需要改造的耦合点（精确到 `文件:行`）、现存缺陷；显式声明「本节记录的是实施前状态」 |
| 二 | 目的 | 是 | 目标能力或新增内容、架构目标、**明确不做（本轮范围外）** |
| 三 | 技术原理 | 是 | 路线选择的依据：已核实的机制事实，逐条标注证据等级（源码核实 / 字节码核实 / 数据包核实 / 实机确认 / 推算），以「为什么选 X 而不是 Y」组织；末尾给「必须遵守的边界」 |
| 四 | 技术路线 | 是 | 分层设计与目标结构、关键类型（API 草案）、数据流、文件改动清单、分阶段实施、文档同步 |
| 五 | 决策记录 | 是 | 编号表（决策点 / 结论），记明被否决的备选与理由，以及**讨论中修正的自身错误留档** |
| 六 | 风险与限制 | 是 | 表格：风险 / 说明 / 处置 |
| 七 | 验证方式 | 是 | 区分「编译验证（开发执行）」与「实机验证（用户执行）」，并列出重点核对项 |
| 八 | 待确认项 | 否 | 尚未裁决或刻意推迟的事项与推荐处置 |
| 九 | 实施结果 | 完成后追加 | 带日期的实施结果：构建结果、产物检查、与计划的偏离、待用户实机验证项 |

- 章节用中文序号（一、二、三…），子节用 `x.y`。
- 事实要么给出证据标注，要么明确标为推断；不写无来源的确定性断言。
- 文件引用精确到 `文件:行`，并注意行号会随实施失效。
- **机制描述不写进计划文档**：实施完成后把机制同步到 `docs/dev/` 对应权威文档，计划文档只保留背景、核实依据与决策记录。
...
> 状态：**进行中**。
> 当前机制的唯一权威描述是 <子系统权威文档路径，如 docs/dev/xxx.md>；本文记录改造背景、核实依据与决策记录。
