...
| `docs/plan/loot-performance/` | 本地观测记录 | **本地目录，不入库**（列在 `.gitignore`）。性能归因的原始日志、逐表 CPU 对照与操作说明，含本机路径与存档摘要 |
...
- **不改向未入库的本地文件**：`CONTEXT.md`、`AGENTS.md`、`CLAUDE.md`、`INSPIRATION.md`、`docs/archive/`、`docs/plan/loot-performance/` 都在 `.gitignore` 里，克隆仓库的人看不到它们。本目录文档必须遵循同样的写法：不得用 markdown 链接指向这些路径——链接在仓库里是死链。需要引用其中的内容时，把必要的那部分**以文本形式写进本文档**，只把本地文件当作本地环境的参考，而不是文档依赖；本地目录本身可以用行内代码写出路径，但不做链接。
