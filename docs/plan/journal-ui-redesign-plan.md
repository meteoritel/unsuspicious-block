...
| R7 | **网格页头部仍由旧 `ScenarioPanel` 绘制**（含它自己的下拉与计算按钮），于是同一下拉会有「旧原生式」与「新浮层式」两份 | **S6 已定**：网格页头部只留读数行与「切换场景 / 计算」，下拉换成与场景页共享的 `ScenarioSelectionOverlay`；旧自绘下拉、以及自 S2 起已无调用点的参数行与幸运值输入框一并删除。见 `docs/archive/journal-scenario-tree-plan.md`（S6 计划，已随实施完成移入本地归档目录；该目录不入库，故此处不提供链接） |
