...
> 状态：**P0 / P1 / P2 已实施并提交，P3 未开工**（2026-09-22 同步；P0、P1 于 2026-09-20 实施，P2 于 2026-09-21 实施，P3「保真与兼容」仍为开放项）。全部设计决定经五轮质询、共 11 批提问与用户逐项确认，决策记录见第五节。实施进度、偏离与验证口径见 [执行侧清单](loottable-scenario-refactor-tasks.md)；机制描述已在实施后同步回 [战利品表系统](../dev/loottable.md) 等 `docs/dev/` 文档，本文只保留改造背景、核实依据与决策记录。
>
...
> 修订（第五轮，2026-09-22 状态同步）：首版状态块「规划完成，尚未实施」已失效——P0 / P1 / P2 均已实施并随提交进入 `1.21.1` 分支，P2 的临时验收 UI 已由前端重构（S1–S6，见 `docs/dev/client-ui.md` 第 4.4–4.6 节）取代；P3（保真与兼容）仍为开放项。本节以上各轮记录保留原文，未做改写。
>
...
3. **文档与代码不一致**（留档）：`docs/archive/loottable-condition-plan.md`（已归档，原 `docs/plan/`）第九节称泥地打捞表的场景数"由 2 降为 1"，而按 `mud_dredging.json:44-75` 与 `plan()` 实际为 3。判定为文档简化/笔误，不影响机制。
...
占合计 **41.2%**。逐表数据与原始日志行已保存至本地观测目录 `docs/archive/loot-performance/`（不入库）。
...
来源 SHA-256、示例组件及完整日志见本地观测目录 `docs/archive/loot-performance/`（不入库）。
...
完整逐表对照、计数及原始证据见本地观测目录 `docs/archive/loot-performance/`（不入库；索引修复对照在其 README 的「19:15 索引修复实机与剩余优化」一节）。
