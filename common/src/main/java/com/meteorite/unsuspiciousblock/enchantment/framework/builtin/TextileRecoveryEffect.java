...
public final class TextileRecoveryEffect implements EnchantmentEffect {
    ...
    public void apply(EffectContext ctx) {
        ...
        if (ctx.triggerContext().targetEntity instanceof Sheep sheep) {
            handleSheepShear(ctx, sheep);
            return;
        }
        ...
        if (state != null && state.is(BlockTags.LEAVES)) {
            handleLeavesDestroy(ctx);
        }
    }

    // 剪羊毛额外掉线
    private void handleSheepShear(EffectContext ctx, Sheep sheep) {
        if (ctx.player().getRandom().nextDouble() >= EXTRA_STRING_CHANCE) {
            return;
        }
        int count = EXTRA_STRING_MIN + ctx.player().getRandom().nextInt(EXTRA_STRING_MAX - EXTRA_STRING_MIN + 1);
        Block.popResource(ctx.level(), sheep.blockPosition(), new ItemStack(Items.STRING, count));
    }

    // 剪树叶额外掉落树叶破坏物品
    private void handleLeavesDestroy(EffectContext ctx) {
        ...
        for (ItemStack drop : Block.getDrops(state, level, pos, null, ctx.player(), ItemStack.EMPTY)) {
            if (!drop.isEmpty()) {
