...
- **日志**：记录实际发现的物品、坐标、维度、群系、结构和时间；可以按时间、维度、群系或备注状态分组，为记录添加备注，并复制传送指令。
...
| 附魔 | 适用物品 | 效果 |
|---|---|---|
| 泥底打捞 I-III | 钓鱼竿 | 钓鱼时有概率打捞额外宝物，每级增加 10% 触发率，在沼泽中额外增加 15% |
...
| Mod Menu | Fabric 配置界面入口 |
| Lootr | NeoForge 构建提供可选兼容，保留每位玩家独立的发掘与日志状态 |

## 配置

- Fabric：`config/unsuspiciousblock/unsuspiciousblock.json`，安装 Mod Menu 后也可从模组列表进入配置界面。
- NeoForge：使用 NeoForge 的模组配置界面与配置文件。
- 可调整内容包括考古战利品表匹配规则、每张表保留的日志数量，以及战利品容器追踪超时。
