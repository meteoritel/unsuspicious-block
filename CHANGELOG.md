...
- fabric 端添加与 Mod Menu 模组的联动

### 变化
- `/usb journal clear` 清除考古笔记数据时，同时清除对应表的已发放奖励标志。

### 修复
- 修正「Completionist's Dust」成就判定范围：从要求整个目录全部解锁，收窄为仅要求原版 6 张考古战利品表（`minecraft:archaeology/` 前缀）全部解锁。
- 全图鉴成就判定改用 rawCatalog，避免服务端启动初期 catalog 渐进模拟导致的判定范围不稳定。
- 新增玩家加入时考古收集类成就补发检测，避免 catalog 未就绪或事件漏触发时遗漏授予。
